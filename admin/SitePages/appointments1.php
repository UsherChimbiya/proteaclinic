    ...
    <main>
        <div class="side-nav">
            <ul class="list-group list-group-flush">
                <a class="list-group-item" href="dashboard.php" data-toggle="tooltip" data-placement="bottom" title="Dashboard"><i class="las la-shapes la-lw"></i><span>dashboard</span></a>
                <a class="list-group-item" href="patients.php" data-toggle="tooltip" data-placement="bottom" title="Patients"><i class="las la-user-injured la-lw"></i><span>patients</span></a>
                                ...
                                <div class="card">
                                    <div class="card-body">
                                        <ul class="list-unstyled">
                                            <?php
                                                $statement1 = $pdo->prepare("SELECT * FROM appointments ORDER BY id DESC");
                                                $statement1->execute();
                                                ...
                                                foreach ($result1 as $row1) {
                                                    $appointmentDate = date("Y-m-d", strtotime($row1['date']));
                                                    
                                            ?>
                                                    <li class="media"><img class="mr-3 rounded-circle" src="../SiteAssets/images/man.svg" alt="..."/>
                                                        <div class="media-body">
                                                            <?php
                                                             if ($currentDate != $appointmentDate) {
                                                                echo '<h5 class="text-muted title">' . date("l, F j, Y", strtotime($row1['date'])) . '</h5>';
                                                                $currentDate = $appointmentDate;
                                                            }
                                                            else {
                                                                 echo '<h5 class="text-muted title"> Today'.  date("l, F j, Y", strtotime($row1['date']));
                                                            }
                                                        // Display the appointment details including
                                                        echo '<b>' . $row1['id'] . '</b> ' . $row1['name'];
                                                        echo '<br>(<b>Email:</b> ' . $row1['email'];
                                                        echo ', <b>Mobile:</b> ' . $row1['mobile'] . ')';
                                                        echo '<br>(<b>Doctor:</b> ' . $row1['doctor'];
                                                        echo ', <b>Date:</b> ' . $row1['date'] . ')';
                                                        echo ', <b>time:</b> ' . $row1['time'] . ')';
                                                        echo ', <b>problem_description:</b> ' . $row1['problem_description'] . ')';
                                                        if ($row1['status'] == "Pending"){
                                                ?>  
                                                <form method="post" action="appointments_approval.php">
                                                   
        
                                                    <a href="approve_form.php"> Approve </a>
                                                </form>  
                                                <?php
                                            } else {
                                                echo ', <b>' . $row1['status'] . '</b>)'; 
                                            }
                                        }
                                                   ?>  
                                                </div>
                                    ...
                                    <div class="card-footer text-center">
                                        <button class="btn btn-dark-red-f"><i class="las la-chevron-circle-down"></i>show more</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <footer>
                    <div class="page-footer text-center">
                                            <div class="fixed-bottom shadow-sm">
                                                <a href="" target="_blank"><img src="../SiteAssets/images/covid-19.svg"/><span>view COVID-19 info</span></a>
                                    </div>
                            </div>
                        </footer>
                    </div></main></body></html>